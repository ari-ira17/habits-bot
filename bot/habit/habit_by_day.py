...
async def set_num_days(message : types.Message, state : FSMContext):
    ...
    await message.answer(
                ...
                f"🫶 Примеры:\n"
                f"Введено {markdown.hbold('«1»')} - повтор каждый день.\n"
                f"Введено {markdown.hbold('«2»')} - повтор каждые два дня.\n"
                f"Введено {markdown.hbold('«7»')} - повтор каждтую неделю.",       
...
async def send_habit_by_day(message: types.Message, data: dict):
    ...
    info_text = (
        f"⚡️ Привычка <b>{title}</b> успешно создана!\n\n"
        f"🔁 Тип: повторение каждые {num_days} день(а)\n"
        f"🕰️ Время напоминания: {time_to_check}\n\n"
